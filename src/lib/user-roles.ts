...
export interface User {
  ...
  name: string;
  email: string;
  role: 'super_admin' | 'admin' | 'parent' | 'teacher';
  child_id?: number;
  is_active: boolean;
  created_at: Date;
  updated_at: Date;
...
export async function createChild(name: string, birthDate: string, createdBy: number) {
  if (!process.env.DATABASE_URL) {
    return { success: false, error: 'No database connection configured' };
  }
...
export async function promoteToAdmin(granterId: number, userId: number, childId: number) {
  if (!process.env.DATABASE_URL) {
    return { success: false, error: 'No database connection configured' };
  }
...
export async function getUserPermissions(userId: number, childId: number) {
  if (!process.env.DATABASE_URL) {
    return { success: false, error: 'No database connection configured' };
  }
